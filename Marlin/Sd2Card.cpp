...
//------------------------------------------------------------------------------
#if DISABLED(SOFTWARE_SPI)  //Ӳ��SPI
  ...
  //------------------------------------------------------------------------------
  /** SPI receive a byte */
  static uint8_t spiRec() { //SPI��1��byte������
    SPDR = 0XFF;
  ...
  void spiRead(uint8_t* buf, uint16_t nbyte) { //SPI��nbyte��byte������
    if (nbyte-- == 0) return;
    SPDR = 0XFF;
...
uint8_t Sd2Card::cardCommand(uint8_t cmd, uint32_t arg) { //SD��д�����
  ...
  if (cmd == CMD12) spiRec(); //�����ֹͣ��̬������,������8������ʱ��

  // wait for response
  for (uint8_t i = 0; ((status_ = spiRec()) & 0X80) && i != 0XFF; i++) { /* Intentionally left empty */ }
  return status_; //status_��ֵ��0b:1xxxx xxxx
}
 ...
 */
bool Sd2Card::eraseSingleBlockEnable() { //��ѯ�Ƿ���������
  csd_t csd;
 ...
 */
bool Sd2Card::init(uint8_t sckRateID, uint8_t chipSelectPin) { //SD����ʼ������
  errorCode_ = type_ = 0;
  ...
  for (uint8_t i = 0; i < 10; i++) spiSend(0XFF);  //���ʹ���74��SCLKʱ��
...
#if ENABLED(SD_CHECK_AND_RETRY)
static const uint16_t crctab[] PROGMEM = {   //CRCУ���
...
bool Sd2Card::readRegister(uint8_t cmd, void* buf) {  //���Ĵ���,cmd������ֵ,buf�Ƿ��ز���
  ...
  if (cardCommand(cmd, 0)) {  //д����
    error(SD_CARD_ERROR_READ_REG);
    goto fail;
  }
  return readData(dst, 16); //��16���ֽڵ�����
fail:
  chipSelectHigh();
  return false;
}
...
bool Sd2Card::readStart(uint32_t blockNumber) {  //��������֪������cmd12��ֹͣ
  if (type() != SD_CARD_TYPE_SDHC) blockNumber <<= 9;
 ...
 * the value zero, false, is returned for failure.
 */
bool Sd2Card::readStop() { //��ֹͣ,���͵���CMD12����
  chipSelectLow();
 ...
 * the value zero, false, is returned for failure.
 */
bool Sd2Card::writeStart(uint32_t blockNumber, uint32_t eraseCount) { //�������д��ʼ
